...
// Gets total number of Teachers
$totalQuery = "SELECT COUNT(*) as total FROM Teacher";
$totalResult = $conn->query($totalQuery);
$totalRows = $totalResult ? $totalResult->fetch_assoc()['total'] : 0;
$totalPages = ceil($totalRows / $rowsPerPage);

// Gets data from database with pagination
$sql = "SELECT * FROM Teacher ORDER BY TeacherID ASC LIMIT ? OFFSET ?";
...
<head>
    ...
    <style>
        ...
        .status-badge-active-teaching { background-color: #28a745; color: white; }
        .status-badge-on-leave { background-color: #ffc107; color: black; }
        .status-badge-temporary { background-color: #17a2b8; color: white; }
        .status-badge-resigned { background-color: #6c757d; color: white; }
    </style>
</head>
<body>
    <!-- Main Content Area -->
    <div class="main-content">
        <div class="content-container">
            ...
            <div class="status-count">
                ...
                <?php 
                // Shows total number of Teachers
                echo "Total number of teachers: " . $totalRows;
                ?>
            </div>
            ...
            <div class="filter-section">
                ...
                <div>
                    <select id="filterClass" class="form-select">
                        ...
                        $classQuery = "SELECT DISTINCT ClassID FROM Teacher WHERE ClassID IS NOT NULL ORDER BY ClassID";
                ...
                <div>
                    <select id="filterStatus" class="form-select">
                        ...
                        <option value="Active Teaching">Active Teaching</option>
                    ...
                    <tbody>
                        ...
                        if ($result && $result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                $statusClass = 'status-badge status-badge-' . strtolower(str_replace(' ', '-', $row['Status'] ?? 'Active Teaching'));
                                ...
                                echo "<td>" . htmlspecialchars($row['ClassID'] ?? 'N/A') . "</td>";
                                
                                // Adds Status column with a custom badge
                                echo "<td><span class='" . htmlspecialchars($statusClass) . "'>" . 
                                     htmlspecialchars($row['Status'] ?? 'Active Teaching') . "</span></td>";
                                
                                echo "<td>
                                    <a href='edit_teacher.php?id=" . htmlspecialchars($row['TeacherID']) . "' class='btn btn-warning btn-sm'><i class='fas fa-edit'></i></a>
                                    <a href='delete_teacher.php?id=" . htmlspecialchars($row['TeacherID']) . "' class='btn btn-danger btn-sm' onclick='return confirm(\"Are you sure you want to delete this teacher?\");'><i class='fas fa-trash'></i></a>
                                </td>";
                                echo "</tr>";
                            }
                        } else {
    ...
    <script>
        ...
        function filterTable() {
            ...
            document.querySelector('.status-count').textContent = `Number of teachers found: ${visibleCount}`;
        }
        ...
        // Checks URL parameters for status messages
